...
class _LanguageSelectView(discord.ui.View):
    ...
    async def select_language(self, select: discord.ui.Select, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.edit_message(content="❌ Translation failed.", view=None)
            return
        ...
        embed.add_field(name=f"Translation ({lang})", value=result, inline=False)
        embed.set_footer(text=f"✅ Language saved | Engine: {translator.name}")
...
async def on_raw_reaction_add(payload: discord.RawReactionActionEvent):
    ...
    if payload.user_id not in _user_lang:
        ...
        await message.reply(f"<@{payload.user_id}> Select your translation language (🌐 will use it automatically next time):", view=view)
        return
...
class _MyLangView(discord.ui.View):
    ...
    async def select_language(self, select: discord.ui.Select, interaction: discord.Interaction):
        ...
        self.stop()
        await interaction.response.edit_message(content=f"✅ Translation language set to **{lang}**", view=None)
